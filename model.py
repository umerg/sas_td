...
class LightSAS(LightningModule):
    def __init__(self, config):
        ...
        self.sas = SASRec(item_num, config)

        #initialisation apparently makes things faster
        for name, param in self.sas.named_parameters():
            try:
                torch.nn.init.xavier_uniform_(param.data)
            except:
                pass # just ignore those failed init layers

        self.embs_n = config.embs_n
        self.sum_weights = torch.nn.Parameter(torch.rand(self.embs_n, device = self.device))
        self.config = config
    ...
    def training_step(self, batch, batch_idx):
        ...
        log_feats = self.sas(log_seqs)
        ##print("Forward ended")
        final_feats = log_feats[:, -self.embs_n:, :]
        sample_embs = self.sas.item_emb(samples).to(self.device) 

        logits_pemb = sample_embs @ final_feats.transpose(-1, -2)

        sample_logits = (logits_pemb @ self.sum_weights.unsqueeze(0).unsqueeze(-1)).squeeze()
        ...
        for param in self.sas.item_emb.parameters(): loss += self.config.l2_emb * torch.norm(param)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        log_feats = self.sas(log_seqs)
        final_feats = log_feats[:, -self.embs_n:, :]  #ONLY THE FINAL EMBEDDING FOR EVAL

        item_embs = self.sas.item_emb(item_indices).to(self.device)

        logits_pemb = item_embs @ final_feats.transpose(-1, -2)
        logits = (logits_pemb @ self.sum_weights.unsqueeze(0).unsqueeze(-1)).squeeze()
    ...
    def test_step(self, batch, batch_idx):
        ...
        #P90 Coverage
        p90_embs = self.sas.item_emb(self.p90_samples).to(self.device)
        logits_pemb = p90_embs.unsqueeze(0) @ final_feats.transpose(-1, -2)
        logits = (logits_pemb @ self.sum_weights.unsqueeze(0).unsqueeze(-1)).squeeze()
